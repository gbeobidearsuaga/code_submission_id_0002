...
#
#
#SBATCH --job-name=va_process
#SBATCH --partition=compute
#SBATCH --time=8:00:00
#SBATCH --output=comment_va_process_%j.log
#SBATCH --error=error_va_process_%j.log
...
for y_id in $(seq ${y_start} ${y_end}); do echo "selected year: $y_id"
		...
		pwd_out=/work/uo1075/u241308/${simulation}/proccessed_output/${var}/
		file_out=${var}_${y_id}_r${r_id}i2p${physics}-LR_26_months_360x180.nc
	...
	cdo merge ${pwd_out}${var}_${y_id}_r*i2p${physics}-LR_26_months_360x180.nc ${pwd_out}${var}_${y_id}_r${r_start}-${r_end}i2p${physics}-LR_26_months_360x180.nc
done
