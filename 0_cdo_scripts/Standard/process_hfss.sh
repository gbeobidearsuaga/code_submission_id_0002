...
#
#
#SBATCH --job-name=hfss_benchmark
#SBATCH --partition=compute
#SBATCH --time=8:00:00
#SBATCH --output=comment_hfss_benchmark_%j.log
#SBATCH --error=error_hfss_benchmark_%j.log
...
for y_id in $(seq ${y_start} ${y_end}); do echo "selected year: $y_id"
		...
		pwd_out=/work/uo1075/u241308/data_python_PostDoc/ML_assimilation/hfss_benchmark/processed/
		file_out=hfss_${y_id}_r${r_id}i2p2-LR_${sel_lead_years}_years_360x180.nc
	...
	cdo merge ${pwd_out}hfss_${y_id}_r*i2p2-LR_${sel_lead_years}_years_360x180.nc ${pwd_out}hfss_${y_id}_r${r_start}-${r_end}i2p2-LR_${sel_lead_years}_years_360x180.nc
done
