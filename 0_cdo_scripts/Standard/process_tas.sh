...
#
#
#SBATCH --job-name=tas_benchmark
#SBATCH --partition=compute
#SBATCH --time=8:00:00
#SBATCH --output=comment_tas_benchmark_%j.log
#SBATCH --error=error_tas_benchmark_%j.log
...
for y_id in $(seq ${y_start} ${y_end}); do echo "selected year: $y_id"
	...
	for r_id in $(seq ${r_start} ${r_end}); do echo "select run: $r_id"
		...
		#file_original1=dkfen4${y_id}_r${r_id}i2p2-LR_echam6_100yr_mm_*.nc
		...
		pwd_out=/work/uo1075/u241308/data_python_PostDoc/ML_assimilation/tas_benchmark/processed/
		file_out=tas_${y_id}_r${r_id}i2p2-LR_${sel_lead_years}_years_360x180.nc

		cdo mergetime ${pwd_original}${file_original1} ${pwd_original}${file_original2} ${pwd_original}${file_original3} ${pwd_out}temp1.nc
		cdo -remapbil,r360x180 -selvar,tas ${pwd_out}temp1.nc ${pwd_out}${file_out}
		rm ${pwd_out}temp1.nc
	done
	cdo merge ${pwd_out}tas_${y_id}_r*i2p2-LR_${sel_lead_years}_years_360x180.nc ${pwd_out}tas_${y_id}_r${r_start}-${r_end}i2p2-LR_${sel_lead_years}_years_360x180.nc
done
