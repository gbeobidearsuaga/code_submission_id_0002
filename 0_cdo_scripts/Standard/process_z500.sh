...
for y_id in $(seq ${y_start} ${y_end}); do echo "selected year: $y_id"
		...
		pwd_out=/work/uo1075/u241308/data_python_PostDoc/ML_assimilation/z500_benchmark/processed/
		file_out=z500_${y_id}_r${r_id}i2p2-LR_${sel_lead_years}_years_360x180.nc

		cdo mergetime ${pwd_original}${file_original1} ${pwd_original}${file_original2} ${pwd_original}${file_original3} ${pwd_out}temp1.nc
		cdo remapbil,r360x180 -sellevel,50000 -selvar,zg ${pwd_out}temp1.nc ${pwd_out}${file_out}
		rm ${pwd_out}temp1.nc
	done
	cdo merge ${pwd_out}z500_${y_id}_r*i2p2-LR_${sel_lead_years}_years_360x180.nc ${pwd_out}z500_${y_id}_r${r_start}-${r_end}i2p2-LR_${sel_lead_years}_years_360x180.nc
done
